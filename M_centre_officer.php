...
class M_centre_officer extends CI_Model {
	...
	public function start(){
		...
		$data['field'] = "*";
		$data['condition'] = "WHERE id_test_centre='$data[sUserTable]' ORDER BY created DESC";
		$data['field_control'] = array(
			/* 'flag_aktif'=>array(
				'replace'=>'',
			), */
			'id_test_centre'=>array(
				//'list'=>select("test_centre","*","WHERE flag_aktif='1' ORDER BY created DESC"),
				//'list_type'=>'1',
				'replace'=>'',
			),
		);
	...
	public function action(){
		...
		if(!empty($action)){
			...
			if($act=='add' || $act=='edit'){
				...
				$input['id_test_centre'] = $data['sUserTable'];
			}
